...
def get_embedding_model() -> HuggingFaceEmbeddings:
    ...
    The model is initialized only once per Python process and
...
def embed_texts_cached(texts: List[str]) -> List[List[float]]:
    ...
    Cache hits are returned immediately without calling the embedding model.
    """
...
def _embed_with_retry(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            logger.warning(
                f"Embedding attempt {attempt} failed: {exc}. "
